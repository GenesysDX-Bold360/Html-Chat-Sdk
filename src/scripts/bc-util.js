...
var useBcvm = window._bcvm && window._bcvm.isIframeEnabled && window._bcvm.isIframeEnabled();
...
bc.AsyncValue = function () {
  var scope = this;
  bc.subscriber(scope, ['success', 'failure']);
  scope.resolve = function (response) {
    scope.fireEvent('success', response);
  };
  scope.reject = function (response) {
    scope.fireEvent('failure', response);
  };
  return scope;
};

bc.AsyncValue.resolve = function (value) {
...
bc.AsyncValue.all = function (arr) {
  var result = new bc.AsyncValue();
  var results = [];
  var resultCount = 0;
  arr.forEach(function (asyncValue, index) {
    asyncValue
 ...
 */
bc.util.createCookie = function (name, value, days, domain) {
  if (useBcvm) {
    window._bcvm.setCookie(name, value, days ? (days * 24 * 60 * 60 * 1000) : null);
    return true;
  } else {
    var expires;
    ...
    if (!domain) {
      domain = window._bcvm && window._bcvm.domain;
    }
    if (!domain) {
      var host = document.location.host;

      // remove port
      var hostIndexOfColon = host.indexOf(':');
      if (hostIndexOfColon !== -1) {
        host = host.substring(0, hostIndexOfColon);
      }
      var hd = host.indexOf('.');
      if (hd !== -1) {
        host = hd == host.lastIndexOf('.') ? '.' + host : host.substring(hd);
      }
 ...
 * @returns {AsyncValue} - The value of the cookie
 */
bc.util.readCookie = function (name) {
  var result = new bc.AsyncValue();
  bc.util.readRawCookie(name).success(function (value) {
    result.resolve(value ? decodeURIComponent(value) : null);
  });
  return result;
};
 ...
 * @param {string} name - The name of the cookie to retrieve
 * @returns {AsyncValue} - The raw value of the cookie
 */
bc.util.readRawCookie = function (name) {
  if (useBcvm) {
    var result = new bc.AsyncValue();
    window._bcvm.getCookieAsync(name).then(result.resolve);
    return result;
  } else {
    ...
    for (var i = 0; i < ca.length; i++) {
      ...
      if (c.indexOf(nameEQ) === 0) {
        return bc.AsyncValue.resolve(c.substring(nameEQ.length, c.length));
      }
    }
...
bc.util.getAbsPathFromRelative = function (winLocation, relativePath) {
  ...
  var sPath = winLocation.pathname.replace(/[^\/]*$/, relativePath.replace(/(\/|^)(?:\.?\/+)+/g, "$1"));
...
bc.util.objectAssign = function (target) {
...
bc.util.subscribeToWindowEvent = function (eventName, eventHandler) {
...
bc.util.unsubscribeFromWindowEvent = function (eventName, eventHandler) {
  if (window.removeEventListener) {
    window.removeEventListener(eventName, eventHandler);
  } else {
    window.detachEvent('on' + eventName, eventHandler);
  }
};
