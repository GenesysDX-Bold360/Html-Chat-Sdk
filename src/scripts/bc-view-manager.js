...
bc.ViewManager = function (formBuilder) {
   ...
   */
  this.setLocalizationValues = function (localizationValues, languageCode) {
    scope.localizer.setLocalizationValues(localizationValues, languageCode);
  };

  this.changeDepartments = function (departments, languageCode) {
    scope.formBuilder.changeDepartments(departments, scope.createdForm, languageCode);
  };
  ...
  /**
   * Callback function invoked each time after outgoing message was sent successfully or not
   * @param {String} messageId    - Message ID
   * @param {Boolean} isDelivered - Boolean flag indicating message delivery status
   */
  // eslint-disable-next-line no-unused-vars
  this.messageDelivered = function (messageId, isDelivered) {
  };
  ...
  /**
   * Hides confirmation dialog UI
   */
  this.hideConfirmationDialog = function () {
    var confirmationDialogs = document.querySelectorAll("div.bc-confirm-dialog");

    for (var i = confirmationDialogs.length - 1; i >= 0; i--) {
      confirmationDialogs[i].parentNode.removeChild(confirmationDialogs[i]);
    }
  }
  ...
  /**
   * Show video session by loading a URL in an iframe or opening it in a popup window.
   * @param {string} url - The url where the visitor should join the session.
   */
  this.showVideoCall = function(url) {
    bc.util.log('Showing video call with visitor URL: ' + url);
  };
