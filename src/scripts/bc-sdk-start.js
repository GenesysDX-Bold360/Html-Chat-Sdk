...
bc.startSession = function (chatParams, visitInfo, hideChatWindow) {
  ...
  function handleFailure() {
    viewManager.hideBusy();
    viewManager.showError('Chat is already ended, please reload');
    bc.currentSession.destroy();
  }
  ...
  if (bc && bc.ViewManager) {
    ...
    bc.currentSession = new bc.Session(bc.config.sessionApiKey, chatParams, visitInfo, viewManager, false);
    bc.currentSession.initializeAsync()
...
bc.openChat = function (chatParams, visitInfo) {
  'use strict';
  bc.AsyncValue.all([bc.util.readCookie(bc.config.configCookie), bc.util.readCookie(bc.config.chatCookie)]).success(function (values) {
    //create the instances
    var siteMetaTag = null;
    var bcMetaTag = null;
    var chatContainerElement = null;
    var configCookieValue = values[0];
    var chatCookieValue = values[1];
      ...
      if (opacity === 0) {
        ...
        if (invites && invites.length > 0) {
          for (var j = 0; j < invites.length; j++) {
            bc.util.removeElement(invites[j]);
          }
        }
      }
    };
    ...
    if (!window.bcChatOpen) {
      if (!bc.config.forcePopup) {
        ...
        if (!chatCookieValue && !configCookieValue) {
